...
# tile_path disimpan relatif terhadap PKG_ROOT (src/oil_palm/), BUKAN ROOT repo —
# supaya sama persis dengan ROOT yang dipakai exp_health.py (dirname dari lokasi
# dirinya sendiri, src/oil_palm/layer1/) saat merekonstruksi path absolut lewat
# os.path.join(ROOT, r.tile_path). Kalau baseline relpath-nya beda, path jadi
# ganda/salah tanpa error yang jelas.
PKG_ROOT = os.path.join(ROOT, "src", "oil_palm")
sys.path.insert(0, os.path.join(PKG_ROOT, "layer1"))
...
def annotations():
    ...
    for a in anns:
        ...
        rows.append((
            m.group(1), fn, os.path.relpath(a["path"], PKG_ROOT),
