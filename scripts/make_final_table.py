"""Bangun docs/FINAL_NUMBERS.md dari docs/RINGKASAN.csv. Jangan ketik tabelnya tangan.

    python scripts/make_final_table.py
...
Tabel ini dihasilkan dari `RINGKASAN.csv` supaya ia mustahil melenceng dari
...
SRC = os.path.join(DOCS, "RINGKASAN.csv")
OUT = os.path.join(DOCS, "FINAL_NUMBERS.md")
...
def main():
    ...
    W("> Dihasilkan `scripts/make_final_table.py` dari `docs/RINGKASAN.csv`. **Jangan sunting tangan.**")
    ...
    def _r(exp, item):
        r = get(rs, exp, item)
        if r is None:
            raise SystemExit("baris hilang di RINGKASAN.csv: %r / %r" % (exp, item))
        return r
    ...
    # RINGKASAN.csv dan TIDAK ADA KODE di paket ini yang menghitung presisi@k.
    ...
    W("`RINGKASAN.csv`, dan tidak ada satu pun skrip di paket ini yang menghitung ")
