...
MODEL AKHIR. `main()` juga melatih SATU model pada SELURUH 5.077 pohon (parameter
`vanilla`, tanpa held-out fold — cross-val di atas hanya untuk mengukur PR-AUC
jujur, bukan untuk memilih model produksi) dan menyimpannya ke
`lightgbm_health/model.txt` + `lightgbm_health/meta.json` (nama fitur, metrik
CV, jumlah pohon latih). Ini satu-satunya artefak yang dipersist skrip ini —
sebelumnya exp_health.py hanya mencetak metrik dan tidak menyimpan apa pun."""
import warnings, os, json, collections, time
...
ROOT = os.path.dirname(BASE)   # PKG_ROOT (src/oil_palm/) — used below only to
                                # resolve `tile_path` values from layer1_crowns.csv,
                                # which build_layer1.py stores relative to this same root.
try:
    from oil_palm.config import DATA_CLEAN_DIR
    CLEAN = os.path.join(DATA_CLEAN_DIR, "layer1_crowns.csv")
except ImportError:
    CLEAN = os.path.join(os.path.dirname(ROOT), "data_clean", "layer1_crowns.csv")
MODEL_DIR = os.path.join(BASE, "lightgbm_health")
...
def main(save=True):
    ...
    prauc_mean, prauc_std, praucs = run(X, y, regs, {**COMMON}, "vanilla")
    print("\n== LightGBM is_unbalance=True ==")
    run(X, y, regs, {**COMMON, "is_unbalance": True}, "unbal")

    # Final production model: same "vanilla" params, trained on ALL 5,077 trees
    # (no held-out fold — the block-CV above is for honest metric reporting only).
    ...
    if save:
        ...
        meta = {
            ...
            "label_map": LABEL,
            "params": {**COMMON},
            "n_trees_trained_on": int(len(y)),
            "n_positive": int(y.sum()),
            "cv_prauc_mean": float(prauc_mean),
            "cv_prauc_std": float(prauc_std),
            "cv_prauc_per_fold": [float(p) for p in praucs],
            "cv_scheme": "leave-one-ortho-out, 3 folds",
            "top_features": [{"name": k, "importance": int(v)} for k, v in imp[:8]],
            "note": ("PR-AUC above is measured leave-one-ortho-out and is UNDERPOWERED "
                     "(66 unique positives). This saved model is trained on all folds "
                     "combined and was never itself evaluated on held-out data."),
        }
        with open(META_PATH, "w", encoding="utf-8") as f:
            json.dump(meta, f, indent=2)
        print(f"\nsaved: {os.path.relpath(MODEL_PATH, ROOT)}, {os.path.relpath(META_PATH, ROOT)}")
