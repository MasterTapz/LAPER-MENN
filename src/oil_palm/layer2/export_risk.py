...
calls output 3:
    ...
    oil_palm/layer2/risk_ranked.csv        one row per palm, ranked
    oil_palm/layer2/risk_ranked.meta.json  provenance sidecar (see below)
...
try:
    from oil_palm.config import FIGURES_DIR as FIGDIR
except ImportError:
    FIGDIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(HERE))), "docs", "figures")
...
def main():
    ...
    # ---- provenance sidecar ---------------------------------------------------
    meta = {
        "produced_by": "oil_palm/layer2/export_risk.py",
