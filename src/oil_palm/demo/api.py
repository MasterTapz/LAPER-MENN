...
Seluruh perhitungan tetap di `core.py`. Berkas ini hanya lapisan HTTP; ia tidak
...
from oil_palm.config import WEB_DIR as WEB, DATA_CLEAN_DIR, LAYER1_DIR, REPO_ROOT as ROOT, ensure_on_path
...
def sick_tiles():
    ...
    if _SICK_TILES is None:
        import csv as _csv
        p = os.path.join(DATA_CLEAN_DIR, "layer1_crowns.csv")
        s = set()
        if os.path.isfile(p):
...
def _banner_bobot():
    ...
    naskah (lihat TAG_ORDER di oil_palm/layer1/detect_centres.py). Perbedaan itu
    sah, tetapi tidak boleh senyap: tanpa banner ini tangkapan layar demo dan
    tabel naskah bisa dikira berasal dari detektor yang sama.
    """
    ensure_on_path()
    ...
    if w is None:
        ...
        print("  Jalankan `python scripts/download_assets.py` dahulu, atau latih ulang")
        print("  dengan scripts/train_layer1.py.")
        return
