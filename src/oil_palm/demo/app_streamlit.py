"""UI demonstrasi SawitGuard-GNN (fallback Streamlit; lihat `api.py`+`web/` untuk versi utama).

    streamlit run src/oil_palm/demo/app_streamlit.py

Berkas ini HANYA menyusun tata letak. Seluruh perhitungan DAN seluruh pembuatan
gambar ada di `core.py`, yang bisa dijalankan sendiri:

    python -m oil_palm.demo.core     # cetak semua angka + render keempat gambar ke PNG
...
with t4:
    ...
    if res is None:
        ...
        with st.expander("Peringatan lengkap yang tersimpan di dalam checkpoint"):
            ...
            st.caption("checkpoint: `oil_palm/layer2/%s`" % res["ckpt"])
