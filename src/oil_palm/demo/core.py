"""Demo logic, WITHOUT Streamlit — so it can be tested without running a server.

`app_streamlit.py` and `api.py` only render; all computation lives here. This
separation is deliberate: if a number on screen is wrong, this is the one file
to check, and it can be run standalone:

    python -m oil_palm.demo.core        # self-test, prints every on-screen number
"""
import os
import sys
...
ensure_on_path()  # puts L1/L2 on sys.path so `import y12`/`import models_real` work
...
def load_risk():
    ...
    if not os.path.isfile(RISK_CSV):
        raise SystemExit(
            "%s tidak ada.\n  jalankan dulu:  python -m oil_palm.layer2.export_risk" % RISK_CSV)
...
def detect_image(path, fold="fold0", conf=0.75, imgsz=640):
    ...
    # `layer1/unhealthy_threshold.py::per_tree_scores`.
    #
...
def sample_images(n=6):
    ...
    crowns = os.path.join(DATA_CLEAN_DIR, "layer1_crowns.csv")
    sick_tiles, clean = [], []
...
def score_photo(df, info, mode="biner"):
    ...
    if not os.path.isfile(V3_CKPT):
        raise SystemExit("%s tidak ada — jalankan python -m oil_palm.layer2.train_final_v3" % V3_CKPT)
...
if __name__ == "__main__":
    ...
    outdir = os.path.join(FIGURES_DIR, "demo_preview")
